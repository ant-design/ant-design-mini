...
class HolidayHandler {
  ...
  isHoliday(date) {
    const currentDate = dayjs(date);
    const holidayMatch = this.holidayList.filter((data) => {
      if (data.range.lenght === 1) {
        const rangeStartDay = data.range[0];
        if (!dayjs(rangeStartDay).isSame(currentDate)) {
          return false;
        }
      } else {
        const startDate = dayjs(data.range[0]);
        const endDate = dayjs(data.range[1]);
        if (currentDate.isBefore(startDate) || currentDate.isAfter(endDate)) {
          return false;
        }
      }
...
class Calendar {
    ...
    constructor(params: any) {
        ...
        this.selectDateList = this.getSelectDateList(selectStartDate, selectEndDate);
    }
