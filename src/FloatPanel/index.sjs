...
function doInit(newVal, oldVal, ownerComponent, instance) {
  const dataset = instance.getDataset() const { id }
...
function handleTouchMove(event, ownerComponent) {
  const { touches, instance }
  const dataset = instance.getDataset() const { id }
 = dataset const { clientY }

  const moveLen = store[`beginY_${id}`] - clientY
  const direction = moveLen > 0 ? DIRECTION.UP: direction.down;directionmoveLen0DIRECTION.UP
...
function handleTouchEnd(event, ownerComponent) {
 ...
 store[`currentHeight_${id}`] === store[`MIDDLE_HEIGHT_${id}`]
        ?;
store`currentHeight_$idstore`MAX_HEIGHT_$idSTATUS_TYPE.TOP status_type.middle: status_type.bottom;
    event.instance.setStyle({
...
store`status_$idSTATUS_TYPE.TOPstore`MAX_HEIGHT_$id`]: store[`MIN_HEIGHT_${id}`];
